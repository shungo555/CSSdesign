...
class gen_numpy():
    ...
    def __init__(self, X, Y, batch_size, patch_size, Ls, sens, nl_range=[0, 30], YBorder=1, data_augmentation=True):
        ...
        self.cfa = cfa_bayer([X.shape[1], X.shape[2]])
        self.cfa_patch = cfa_bayer([self.patch_size[0], self.patch_size[1]])
    ...
    def generator(self):
        ...
        while(True):
            for i in range(self.batch_size):
                ...
                mosaic1[i, :, :, :] = create_raw(x[i, :, :, :], self.sens, self.Ls, self.cfa_patch)
                mosaic1[i, :, :, :] += noise[i, :, :, :]
                gains[i, 0] = self.gains[k, 0]
                mosaic1[i, :, :, :] *= gains[i, 0]
            yield mosaic1, yy, noise, gains
